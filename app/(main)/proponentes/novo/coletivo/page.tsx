...
const STEPS = [
    { id: 'dadosBasicos', title: 'Dados Básicos', sections: ['dadosColetivo', 'contato'] },
    { id: 'responsavel', title: 'Responsável Legal', sections: ['responsavel', 'enderecoResponsavel'] },
    { id: 'enderecoColetivo', title: 'Endereço do Coletivo', sections: ['enderecoPessoaJuridica'] },
...
const initializeFormData = () => {
    ...
    // Get all sections from proponenteColetivoForm
    const allSections = [
        proponenteColetivoForm.dadosColetivo,
        proponenteColetivoForm.contato,
        proponenteColetivoForm.responsavel,
        proponenteColetivoForm.enderecoResponsavel,
        proponenteColetivoForm.enderecoPessoaJuridica,
        proponenteColetivoForm.perfilDoResponsavel.informacoesDemograficas,
        proponenteColetivoForm.perfilDoResponsavel.experiencia,
        proponenteColetivoForm.perfilDoResponsavel.aspectosFinanceiros,
        proponenteColetivoForm.perfilDoResponsavel.objetivos,
    ];
...
export default function ColetivoPage() {
    ...
    const getCurrentFields = () => {
        ...
        step.sections.forEach(sectionPath => {
            ...
            if (sections.length === 1) {
                fields = (proponenteColetivoForm as any)[sections[0]];
            } else if (sections.length === 2) {
                fields = (proponenteColetivoForm as any)[sections[0]][sections[1]];
            }
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            proponenteColetivoForm.dadosColetivo.forEach((field: any) => {
                organizedData.dadosColetivo[field.name] = processFieldValue(field, formData[field.name]);
            });

            proponenteColetivoForm.contato.forEach((field: any) => {
                organizedData.contato[field.name] = processFieldValue(field, formData[field.name]);
            });

            proponenteColetivoForm.responsavel.forEach((field: any) => {
                organizedData.responsavel[field.name] = processFieldValue(field, formData[field.name]);
            });

            proponenteColetivoForm.enderecoResponsavel.forEach((field: any) => {
                organizedData.enderecoResponsavel[field.name] = processFieldValue(field, formData[field.name]);
            });

            proponenteColetivoForm.enderecoPessoaJuridica.forEach((field: any) => {
                organizedData.enderecoPessoaJuridica[field.name] = processFieldValue(field, formData[field.name]);
            });

            proponenteColetivoForm.perfilDoResponsavel.informacoesDemograficas.forEach((field: any) => {
                organizedData.perfilDoResponsavel.informacoesDemograficas[field.name] = processFieldValue(field, formData[field.name]);
            });

            proponenteColetivoForm.perfilDoResponsavel.experiencia.forEach((field: any) => {
                organizedData.perfilDoResponsavel.experiencia[field.name] = processFieldValue(field, formData[field.name]);
            });

            proponenteColetivoForm.perfilDoResponsavel.aspectosFinanceiros.forEach((field: any) => {
                organizedData.perfilDoResponsavel.aspectosFinanceiros[field.name] = processFieldValue(field, formData[field.name]);
            });

            proponenteColetivoForm.perfilDoResponsavel.objetivos.forEach((field: any) => {
